...
def get_or_create_whatsapp_session(phone_number):
    """Get or create WhatsApp session with location persistence"""
    ...
    if session_key not in user_sessions:
        ...
        if WhatsAppSession and ChatSession:
            try:
                ...
                logger.warning(f"Could not create WhatsApp session in database: {str(e)}")
...
def save_location_to_session(phone_number, location):
    """Save location to both memory and database"""
    session_key = phone_number.replace('whatsapp:', '')
...
def process_whatsapp_message(from_number, message_body):
    """Process WhatsApp message and return response with improved session management"""
    try:
        # Get or create session with location persistence
        ...
        if message_body.lower() in ['hi', 'hello', 'hey', 'start']:
            # Keep location but reset conversation flow
            location_memory = session.get('location')
            session.update({
            ...
            if location_memory:
                # If location is already set, skip location request
...
def handle_welcome_state(from_number, message_body, session):
    ...
    if any(greeting in message_body.lower() for greeting in greetings):
        ...
        if session.get('location'):
            # Location already set, go directly to chatting
        ...
        # If not a greeting, treat as symptom and set default location if needed
        session['state'] = 'chatting'
