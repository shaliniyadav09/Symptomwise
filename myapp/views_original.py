...
def bookappointment(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Create appointment directly in database to handle all fields
            ...
            cursor.execute("""
                INSERT INTO adminapp_appointment (
                    appointment_id, first_name, last_name, phone, email,
                    zipcode, dob, date_of_birth, gender, appointment_date, appointment_time,
                    reason, status, address, doctor_id, hospital_id,
                    duration, consultation_fee, is_paid, payment_method,
                    patient_notes, doctor_notes, symptoms, created_at, updated_at,
                    patient_user_id, confirmed_at
...
def get_doctors_by_hospital(request):
    """Get doctors by hospital (AJAX endpoint) - Fixed to avoid database errors"""
    if request.method == 'POST':
